...
class ToolDocumentationExtractor:
        ...
        if hasattr(annotation, "__name__"):
            ...
            if origin is Union:
                ...
                if annotation.__args__:
                    args_str = ", ".join(
                        self._format_type_annotation(arg) for arg in annotation.__args__
                    )
    ...
    def _format_default_value(self, default) -> str | None:
...
The MCP Server for Splunk provides {len(self.extractor.tools_metadata)} production-ready tools organized into categories for different use cases including search operations, system administration, data management, and health monitoring."""
    ...
    def _generate_overview(self) -> str:
        ...
        for category, count in sorted(categories.items()):
            overview_parts.append(
                f"- **{category.title()}** ({count} tools) - {self._get_category_description(category)}"
            )
        ...
        for category in sorted(categories.keys()):
            ...
            for tool_name, info in tools:
                ...
                if tool_name in self.extractor.method_signatures:
                    ...
                    if params:
                        required_params = [
                            name for name, info in params.items() if info["required"]
                        ]
                        optional_params = [
                            name for name, info in params.items() if not info["required"]
                        ]
                        ...
                        if optional_params:
                            doc_parts.append(
                                f"**Optional Parameters:** {', '.join(f'`{p}`' for p in optional_params)}"
                            )
                    else:
    ...
    def _generate_tool_detail(self, tool_name: str) -> str:
        ...
        parts.extend(["", "#### Description", "", metadata.description])
        ...
        if tool_name in self.extractor.method_signatures:
            ...
            if params:
                ...
                for param_name, param_info in params.items():
                    ...
                    parts.append(
                        f"| `{param_name}` | `{param_info['type']}` | {required} | `{default}` | {description} |"
                    )
            else:
                parts.append("This tool takes no parameters.")

            # Return value section
            parts.extend(["", "#### Returns", "", f"**Type:** `{sig_info['return_type']}`", ""])
            ...
            if "returns" in parsed_doc:
                ...
                parts.append(
                    "Returns a dictionary containing the operation result with success status and data."
                )

            # Usage example
            parts.extend(["", "#### Usage Example", "", "```python"])
            ...
            if "example" in parsed_doc or "examples" in parsed_doc:
                ...
                if example_content:
                    parts.extend(["", "#### Additional Examples", "", example_content])
...
class OpenAPIGenerator:
    ...
    def generate_openapi_spec(self) -> dict[str, Any]:
        ...
        spec = {
            ...
            "servers": [{"url": "http://localhost:8001/mcp", "description": "Development server"}],
            "paths": {
                "/tools/call": {
                    "post": {
            ...
            "components": {"schemas": self._generate_schemas()},
        }
