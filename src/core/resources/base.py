...
from ..client_identity import ClientIdentity, get_client_manager, SecurityError
...
class ClientScopedResource(BaseResource):
    ...
    def __init__(self, base_uri_template: str, name: str, description: str, mime_type: str = "text/plain"):
    ...
    async def get_content(self, ctx: Context, uri: str = None) -> str:
        ...
        if not client_config:
            raise SecurityError("No client configuration found - resources require client-specific Splunk access")
        ...
        try:
            ...
            self.logger.error(f"Failed to get resource content for client {identity.client_id}: {e}")
            raise
    ...
    def _get_client_config_from_context(self, ctx: Context) -> Optional[Dict[str, Any]]:
    ...
    def _validate_client_uri_access(self, identity: ClientIdentity, uri: str):
        ...
        if identity.client_id not in uri:
            raise SecurityError(f"Access denied: URI {uri} does not belong to client {identity.client_id}")
        
        # Additional URI validation can be added here
        self.logger.debug(f"URI access validated for client {identity.client_id}: {uri}")
    
    @abstractmethod
    async def _get_client_specific_content(self, identity: ClientIdentity, service: client.Service, uri: str) -> str:
        """
