...
class EnhancedConfigExtractor:
    ...
    async def extract_client_config(self, ctx: Context, tool_params: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        self.logger.debug(f"Context attributes: {[attr for attr in dir(ctx) if not attr.startswith('_')]}")
    ...
    def _extract_from_context_variable_headers(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if headers:
                ...
                if config:
                    self.logger.debug("Successfully extracted Splunk config from context variable headers")
                    return config
            else:
    ...
    def _extract_from_middleware_context(self, ctx: Context) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            potential_attrs = ['splunk_client_config', 'client_config', 'middleware_data', 'request_data']
    ...
    def _extract_from_http_headers(self, ctx: Context) -> Optional[Dict[str, Any]]:
    ...
    def _extract_from_mcp_metadata(self, ctx: Context) -> Optional[Dict[str, Any]]:
    ...
    def _extract_from_config_file(self, ctx: Context) -> Optional[Dict[str, Any]]:
    ...
    def _extract_from_environment(self, ctx: Context) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(ctx.request_context, 'lifespan_context'):
                ...
                if hasattr(splunk_ctx, 'client_config') and splunk_ctx.client_config:
                    ...
                    self.logger.debug(f"No client_config in lifespan context. Available attributes: {[attr for attr in dir(splunk_ctx) if not attr.startswith('_')]}")
            else:
    ...
    def _extract_from_websocket_session(self, ctx: Context) -> Optional[Dict[str, Any]]:
    ...
    def _extract_from_client_certificate(self, ctx: Context) -> Optional[Dict[str, Any]]:
        """Extract config from client certificate metadata"""
        try:
            # Check if client provided a certificate with embedded metadata
            if hasattr(ctx.request_context, 'request') and hasattr(ctx.request_context.request, 'scope'):
    ...
    def _extract_from_json_payload(self, ctx: Context) -> Optional[Dict[str, Any]]:
        """Extract config from JSON payload in request body"""
        try:
            # Check if request has a JSON body with configuration
            if hasattr(ctx.request_context, 'request') and hasattr(ctx.request_context.request, 'json'):
    ...
    def _get_client_identifier(self, ctx: Context) -> Optional[str]:
        ...
        try:
            ...
            if hasattr(ctx.request_context, 'request'):
                ...
                if hasattr(request, 'headers'):
                    ...
                    client_id = (headers.get('x-client-id') or 
                                headers.get('x-mcp-client-id') or
    ...
    def _extract_config_from_headers(self, headers: Dict[str, str]) -> Optional[Dict[str, Any]]:
