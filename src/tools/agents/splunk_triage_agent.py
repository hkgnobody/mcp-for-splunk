...
1. **Start with basics**: Check Splunk edition, license, and obvious issues
2. **Index verification**: Confirm data location and access permissions
3. **Time analysis**: Rule out time range and timestamp issues
4. **Infrastructure**: Verify forwarders, search heads, and connectivity
5. **Search validation**: Check query syntax and field extractions
6. **Advanced diagnostics**: Deep dive into specific component issues
...
Document your findings at each step and provide actionable recommendations.
        ...
        logger.debug("Missing data specialist agent created with official workflow")
        return agent
...
Always explain your reasoning and cite specific metrics or log entries.
        ...
        logger.debug("Inputs specialist agent created with model and tools configured")
        return agent
...
3. Examine search concurrency and scheduling
4. Investigate queue sizes and processing delays
5. Correlate performance with system events
...
Provide specific tuning recommendations with expected impact.
        ...
        logger.debug("Performance specialist agent created with model and tools configured")
        return agent
...
Your approach:
1. Analyze indexing delay patterns and trends
2. Examine bucket management and storage utilization
3. Investigate parsing efficiency and transformation costs
4. Check cluster health and replication status
5. Review retention policies and storage allocation
6. Provide indexing optimization strategies
...
Provide specific configuration recommendations for optimal performance.
        ...
        logger.debug("Indexing specialist agent created with model and tools configured")
        return agent
...
Provide clear, actionable guidance for common problems.
        ...
        logger.debug("General specialist agent created with model and tools configured")
        return agent
        ...
        try:
            ...
            try:
                result = await Runner.run(
                    ...
                    max_turns=10  # Allow multiple turns for handoffs
                )
            finally:
