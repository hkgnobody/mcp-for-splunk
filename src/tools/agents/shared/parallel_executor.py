...
class ParallelWorkflowExecutor:
    ...
    async def _execute_workflow_core(
        ...
        try:
            ...
            await ctx.info(f"🔄 All phases completed, finalizing results...")
    ...
    def _parse_agent_result_to_diagnostic(self, agent_result: Any, task: TaskDefinition) -> DiagnosticResult:
        ...
        if 'error' in output_lower or 'failed' in output_lower or 'critical' in output_lower:
            status = "critical"
        elif 'warning' in output_lower or 'issue' in output_lower or 'problem' in output_lower:
            status = "warning"
        else:
            status = "healthy"
