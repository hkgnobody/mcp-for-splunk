...
class ToolValidator:
    ...
    def _validate_class_structure(self, tree: ast.AST, content: str):
        ...
        if len(tool_classes) > 1:
            self.warnings.append(f"Multiple BaseTool classes found: {[cls.name for cls in tool_classes]}")
    ...
    def _validate_documentation(self, tree: ast.AST, content: str):
        ...
        if todo_count > 5:
            self.warnings.append(f"Many TODO comments found ({todo_count}) - consider completing implementation")
    ...
    def _validate_error_handling(self, tree: ast.AST, content: str):
        ...
        if 'format_error_response' not in content:
            self.warnings.append("Consider using format_error_response for consistent error handling")

        if 'format_success_response' not in content:
            self.warnings.append("Consider using format_success_response for consistent response formatting")
    ...
    def _validate_test_file(self, tool_file: Path):
        ...
        try:
            ...
            if '@pytest.mark.asyncio' not in test_content:
                self.warnings.append("Test file should include async tests with @pytest.mark.asyncio")
...
def validate_all_tools(contrib_dir: Path) -> dict[str, dict[str, list[str]]]:
    ...
    for category_dir in tools_dir.iterdir():
        ...
        for tool_file in category_dir.iterdir():
            if (tool_file.is_file() and
                tool_file.suffix == '.py' and
                not tool_file.name.startswith('_')):
