...
async def test_missing_data_scenario():
    ...
    try:
        ...
        if "context_inspection" in result:
            ...
            print(
                f"   • Input length: {context_info['orchestration_input']['total_length']} characters"
            )
            print(
                f"   • Available specialists: {context_info['agent_context']['available_specialists']}"
            )
            print(
                f"   • Context efficiency: {context_info['context_optimization']['context_efficiency_score']:.2f}"
            )
...
async def test_performance_scenario():
    ...
    try:
        ...
        if "context_inspection" in result:
            ...
            print(
                f"   • Performance specialists: {len([s for s in context_info['agent_context']['specialist_names'] if 'Performance' in s or 'Resource' in s or 'Concurrency' in s])}"
            )
