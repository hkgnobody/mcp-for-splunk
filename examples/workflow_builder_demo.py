...
**Output:** Return DiagnosticResult with privilege escalation findings.
        ...
        try:
            result1 = await workflow_builder.execute(
                ctx=ctx,
                mode="process",
                workflow_data=finished_workflow_dict
            )
            ...
            print(f"🏃 Compatible with Runner: {result1['processing_metadata']['compatible_with_runner']}")
            print(f"📋 Total Tasks: {result1['validation']['summary']['total_tasks']}")
            print(f"🔧 Required Tools: {', '.join(result1['validation']['summary']['required_tools'])}")
            print(f"📝 Context Variables: {', '.join(result1['validation']['summary']['context_variables'])}")
            
            if result1['integration_ready']:
                print("\n💡 Usage Instructions:")
                print(f"   - Workflow Runner: {result1['usage_instructions']['workflow_runner']}")
                print(f"   - Dynamic Troubleshoot: {result1['usage_instructions']['dynamic_troubleshoot']}")
...
**Output:** Return DiagnosticResult with data integrity findings.
        ...
        try:
            result2 = await workflow_builder.execute(
                ctx=ctx,
                mode="process",
                workflow_data=finished_workflow_json
            )
        ...
        try:
            ...
            if result3['validation']['suggestions']:
                print(f"\n💡 Fix Suggestions:")
                for suggestion in result3['validation']['suggestions'][:3]:  # Show first 3 suggestions
                    print(f"   - {suggestion}")
        ...
        try:
            ...
            if validation_result['status'] == 'success':
                ...
                print(f"   📊 Summary: {validation_result['summary']['total_tasks']} tasks, {len(validation_result['summary']['required_tools'])} tools")
        ...
        try:
            ...
            create_result = await workflow_builder.execute(
                ctx=ctx,
                mode="create"
            )
...
async def demonstrate_workflow_requirements():
    ...
    try:
        ...
        if schema_result['status'] == 'success':
            ...
            print(f"   ✅ WorkflowDefinition Schema: {len(schemas['WorkflowDefinition']['properties'])} properties")
            print(f"   📋 TaskDefinition Schema: {len(schemas['TaskDefinition']['properties'])} properties")
...
if __name__ == "__main__":
    ...
    print("   3. Process: result = await tool.execute(ctx=context, mode='process', workflow_data=your_workflow)")
